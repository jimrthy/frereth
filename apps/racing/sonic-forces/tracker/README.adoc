...
now uses Pathom for server-side mutation and query handling.  See `tracker/server_components/pathom_wrappers.clj` and
`tracker/model/user.clj` for the macro definitions and examples of how to define
...
`src/main/tracker/client.cljs` contains the code to start and refresh
...
See the server section below for working on the full-stack tracker itself.
...
* (there's an extra lein vs. shadow step in here)
...
In a .clj buffer, use `C-c C-s b` and choose the clj REPL so you can
work with the current buffer.

There must be a better way, but is this the first I've found that works.
...
application `tracker.development-preload`. You can add code here that
